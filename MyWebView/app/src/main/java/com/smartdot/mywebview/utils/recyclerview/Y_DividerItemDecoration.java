...
public abstract class Y_DividerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount1; i++) {
            ...
            int xlineWidthPx = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, divider.getXlineWidthDp(), context.getResources().getDisplayMetrics());
            int ylineWidthPx = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, divider.getYlineWidthDp(), context.getResources().getDisplayMetrics());

            if (divider.getLeft()) {
                drawChildLeftVertical(child, c, parent, divider.getColorARGB(), xlineWidthPx);
            }
            if (divider.getTop()) {
                drawChildTopHorizontal(child, c, parent, divider.getColorARGB(), ylineWidthPx);
            }
            if (divider.getRight()) {
                drawChildRightVertical(child, c, parent, divider.getColorARGB(), xlineWidthPx);
            }
            if (divider.getBottom()) {
                drawChildBottomHorizontal(child, c, parent, divider.getColorARGB(), ylineWidthPx);
            }
        }
    }
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        int left = divider.getLeft() ? xlineWidthPx : 0;
        int top = divider.getTop() ? ylineWidthPx : 0;
        int right = divider.getRight() ? xlineWidthPx : 0;
        int bottom = divider.getBottom() ? ylineWidthPx : 0;
