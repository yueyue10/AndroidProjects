...
public class BadgeUtil {
	 ...
	 * @param count
	 */
	private static void sendToXiaoMi(Notification notification,
			Context context, int count) {
		try {
			// Class miuiNotificationClass =
			// Class.forName("android.app.MiuiNotification");
			// Object miuiNotification = miuiNotificationClass.newInstance();
			// Field field =
			// miuiNotification.getClass().getDeclaredField("messageCount");
			// field.setAccessible(true);
			// field.set(miuiNotification, String.valueOf(count == 0 ? "" :
			// count)); // 设置信息数-->这种发送必须是miui 6才行
	 ...
	 * @param context
	 */
	public static void resetBadgeCount(Notification notification,
			Context context) {
